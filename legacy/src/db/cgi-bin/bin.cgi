...
if ($$params{action} eq 'oob') {
  ...
  &printTitles($tbl, $cgi);
  for (@$rows) {
    &printRow($tbl, $cgi, $user, $_, { cutoff=>($$params{cutoff}?undef:$CUTOFF), row=>$row, oob=>1 });
    $row = not $row;
  }
  print "</table>\n\n";
...
########################################################################
# set a single album to be out-of-bin in DB
# ADDED: by cfperron and mcbaron on 10/15/2012 per jkrug's request <3
########################################################################
}  elsif ($$params{action} eq 'OOBSingle') {
  $$params{sec} = 'Bin' unless $$params{sec};
  my %oob = (Bin	=> 'OOB',
	     'N&WC'	=> 'NIB',
	     TBR	=> 'NIB');
  ...
  &sqlUpdate($tbl, {DateRemoved=>$date, Status=>$oob{$$params{sec}}},
	     {string=>'AlbumID = ?',
	      values=>[$$params{AlbumID}]},
	     $$params{debug});
  print "<h2>Album $$params{AlbumID} has been changed to $oob{$$params{sec}}</h2>\n\n";
