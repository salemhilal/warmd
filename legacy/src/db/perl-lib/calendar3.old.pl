...
sub calendarGen {
  ...
  foreach my $d (0..6) {
    foreach my $h (0..24) {
      foreach my $m (0,30) {
        $$calendar[$d]->[$h]->[$m] = {Type=>'dead', Duration=>24-$h, Program=>'Dead Air'};
      }
    }
  }
  ...
  foreach my $event (@$rows) {
    ...
    #$em = (($em > 0 && $em < 30) ? 0 : 30); # any end minute greater than 0 snap to 30 min mark
    ...
    #$minutes = ($em != $sm) ? 30 : 0;
    $$event{DurationH} = $hours;
    $$event{DurationM} = $minutes;

    # shorten the duration of all preceding dead time
    if ($sh > 0 and $$calendar[$sDOW]->[$sh-1]->[$sm]->{Type} eq 'dead') {
      my $dead = $sh;
      my $deadm = $sm;
      while ($dead>0 and $$calendar[$sDOW]->[$dead-1]->[$deadm]->{Type} eq 'dead') { 
        $dead--;
        $$calendar[$sDOW]->[$dead]->[$deadm]->{DurationH} = $sh-$dead;
