...
if ($$params{mode} eq 'rows') {
  ...
  my @mytime = ($$params{Time} =~ /(\d{4})-(\d\d)-(\d\d) (\d\d):(\d\d):(\d\d)/);
  my $timewhere = {string=>'YEAR(Time) = ? AND MONTH(Time) = ? AND DAYOFMONTH(Time) = ? AND HOUR(Time) = ? AND MINUTE(Time) = ? AND SECOND(Time) = ?', values=>[@mytime]};
